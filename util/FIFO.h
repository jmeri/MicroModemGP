...
static inline void fifo_push(FIFOBuffer *f, unsigned char c) {
  *(f->tail) = c;
...
static inline unsigned char fifo_pop(FIFOBuffer *f) {
  if(f->head == f->end) {
    f->head = f->begin;
    return *(f->end);
  } else {
...
static inline void fifo_init(FIFOBuffer *f, unsigned char *buffer, size_t size) {
